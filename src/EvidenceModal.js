...
function EvidenceModal(props) {
    ...
    //             if (f.name === res.req._data.get('key').split('/').at(-1)) {
    //                 f.completed = true
    //             }
    //             return f
    //         });
    ...
    //             }
    //             postData.append('file', acceptedFiles.filter((f) => f.name === file.data.fields.key.split('/').at(-1))[0])
    ...
    //         request
    //             .post('/sign_s3')
    //             .send({ files: fileArgs, topic_id: props.topicId })
    ...
    useEffect(() => {
        const allUploaded = files.length > 0 && files.every(file => uploadProgress[file.name] === 100);
        if (allUploaded) {
